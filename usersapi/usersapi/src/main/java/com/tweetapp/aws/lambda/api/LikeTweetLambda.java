...
public class LikeTweetLambda {
	...
	public APIGatewayProxyResponseEvent likeTweet(APIGatewayProxyRequestEvent requestGot) 
		...
		try {
			...
			tweet.likes.add(username);
			List<String> updatedLikes = new ArrayList<>();
			updatedLikes.add(username);
			updatedLikes.addAll(tweet.likes);
			UpdateItemSpec updateItemSpec = new UpdateItemSpec().withPrimaryKey("tweetId", tweetID)
	                .withReturnValues(ReturnValue.ALL_NEW).withUpdateExpression("set likes = :val1")
	                .withValueMap(new ValueMap().withList(":val1", updatedLikes));
